...
const FoodPosts = () => {
  ...
  // Filter to get only the food category
  const FoodCategory = categories.find(category => category.categoryName === 'Food');

  return (
    <div>
        {FoodCategory && (
          <div key={FoodCategory._id} className='rounded-3xl shadow-md border border-gray-300 mt-10'>
            <div className='flex items-center justify-between mx-10'>
              <h1 className='text-3xl font-semibold text-center mt-5'>{FoodCategory.categoryName} Posts</h1>
              <Link to='/food' className='hover:underline font-medium mt-5'>See More of Food Posts</Link>
            </div>
            <div className='grid grid-cols-3 gap-8 h-[50%] mx-10 my-10 py-6'>
              {posts.filter(p => p.category === FoodCategory._id).slice(0, 3).map(post => (
                <div key={post._id} className='border border-gray-500 rounded-lg shadow-lg p-4 hover:-translate-y-2 duration-300 h-96'>
                  ...
                  <p className='my-1'>
                    <span className='font-semibold text-[18px]'>Category:</span> {FoodCategory.categoryName}
                  </p>
                  <p className='my-1'>
