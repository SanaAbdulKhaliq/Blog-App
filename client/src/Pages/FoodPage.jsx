...
const FoodPage = () => {
  ...
  return (
    <div>
      <Topbar />
      <div className='bg-food-image bg-cover h-[90vh]'>
        <div className='text-white flex flex-col gap-5 text-center relative top-[40%]'>
          <h1 className='text-5xl font-semibold'>Adventures in Flavor</h1>
          <p>Exploring the culinary world.</p>
        </div>
      </div>

      <div>
        {foodCategory && (
          <div key={foodCategory._id} className='mt-10'>
            <h1 className='text-2xl font-semibold text-center mt-5'>{foodCategory.categoryName} Posts</h1>
            <div className='grid grid-cols-3 gap-8 h-96 mx-10 my-10 py-6'>
              {posts.filter(p => p.category === foodCategory._id).map(post => (
